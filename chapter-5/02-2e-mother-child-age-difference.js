...
function average(values) {
  ...
  return values.reduce(plus) / values.length;
}
...
const peopleWithMothers = ANCESTRY_FILE.filter(p => p.mother !== null);

const byName = ANCESTRY_FILE.reduce((acc, person) => {
  const { name } = person;
...
function averageAgeDifference(people) {
  return average(
    people
      .map(person => {
        const { mother, name } = person;

        if (byName[mother]) {
          return byName[name].born - byName[mother].born;
        }
...
console.log(averageAgeDifference(peopleWithMothers));
